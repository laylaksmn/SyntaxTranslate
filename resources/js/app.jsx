...
function SelectLang({ value, onChange }) {
    return (
        <div className="relative">
            ...
            <ChevronDown size={14} strokeWidth={3} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-900 pointer-events-none" />
        </div>
    );
}
            ...
            <div className="flex flex-1 gap-6 relative">
                ...
                <main className="flex-1 flex flex-col pb-8 min-w-0">
                    <div className="flex justify-between items-center mb-8 mt-2">
                        <h1 className="text-3xl font-black tracking-tight text-slate-900">TRANSLATE_SYNTAX</h1>
                        <button 
                            onClick={click}
                    ...
                    <div className="flex flex-1 gap-6">
                        ...
                        <div className="flex-1 bg-[#f4f5f7] rounded-[2rem] p-5 flex flex-col border border-slate-200/60 shadow-sm">
                            <div className="flex justify-between items-center mb-4 px-3 mt-1">
                                <div className="flex items-center gap-3">
                                    <span className="text-[11px] font-bold text-slate-600 tracking-widest">INPUT_BLOCK</span>
                                    <SelectLang value={sourceLang} onChange={setSourceLang} />
                                </div>
                                <div className="flex gap-4 text-slate-500">
                                    <button onClick={() => setSourceCode("")} className="hover:text-slate-900 transition-colors cursor-pointer"><Trash2 size={18} /></button>
                                    <button onClick={() => handleCopy(sourceCode)} className="hover:text-slate-900 transition-colors cursor-pointer"><Copy size={18} /></button>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 bg-[#f4f5f7] rounded-[2rem] p-5 flex flex-col border border-slate-200/60 shadow-sm">
                            <div className="flex justify-between items-center mb-4 px-3">
                                <div className="flex items-center gap-3">
                                    <span className="text-[11px] font-bold text-slate-600 tracking-widest">OUTPUT_BLOCK</span>
                                    <SelectLang value={targetLang} onChange={setTargetLang} />
                                </div>
                                <div className="flex gap-4 text-slate-500">
                                    <button onClick={() => handleCopy(resultCode)} className="hover:text-slate-900 transition-colors cursor-pointer"><Copy size={18} /></button>
                                    <button 
                                        onClick={() => {
                                            if (!resultCode) return;
                                            const element = document.createElement("a");
                                            const file = new Blob([resultCode], {type: 'text/plain'});
                                            element.href = URL.createObjectURL(file);
                                            element.download = `translated.${targetLang}`;
                            ...
                            <div className="flex justify-between items-center mt-5 px-3 mb-1 text-[10px] font-bold tracking-widest">
                                <span className="text-slate-500">{targetLang.toUpperCase()}</span>
                                {resultCode && (
                                    <span className="flex items-center gap-1.5 text-emerald-600">
                                        <span className="text-[9px]">●</span> TRANSLATED_SUCCESSFULLY
                                    </span>
                                )}
                            </div>
                        </div>
